...
static int EccPoint_compute_pub_key(EccPoint *result, uECC_word_t *priv) {
    uECC_word_t tmp1[uECC_WORDS];
    uECC_word_t tmp2[uECC_WORDS];
    uECC_word_t *p2[2] = {tmp1, tmp2};
    uECC_word_t carry;

    /* Make sure the priv key is in the range [1, n-1]. */
    if (vli_isZero(priv)) {
        return 0;
    }
    ...
    EccPoint_mult(result, &curve_G, priv, 0, vli_numBits(priv, uECC_WORDS));
#else
    if (vli_cmp(curve_n, priv) != 1) {
        return 0;
    }

    // Regularize the bitcount for the priv key so that attackers cannot use a side channel
    // attack to learn the number of leading zeros.
    carry = vli_add(tmp1, priv, curve_n);
...
int uECC_make_key(uint8_t pub_key[uECC_BYTES*2], uint8_t priv_key[uECC_BYTES]) {
    uECC_word_t priv[uECC_WORDS];
    EccPoint pub;
    uECC_word_t tries;
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        if (g_rng_function((uint8_t *)priv, sizeof(priv)) &&
                EccPoint_compute_pub_key(&pub, priv)) {
            vli_nativeToBytes(priv_key, priv);
            vli_nativeToBytes(pub_key, pub.x);
            vli_nativeToBytes(pub_key + uECC_BYTES, pub.y);
            return 1;
        }
    }
    return 0;
...
int uECC_shared_secret(const uint8_t pub_key[uECC_BYTES*2],
                       const uint8_t priv_key[uECC_BYTES],
                       uint8_t secret[uECC_BYTES]) {
    EccPoint pub;
    EccPoint product;
    uECC_word_t priv[uECC_WORDS];
    uECC_word_t tmp[uECC_WORDS];
    uECC_word_t *p2[2] = {priv, tmp};
    ...
    vli_bytesToNative(priv, priv_key);
    vli_bytesToNative(pub.x, pub_key);
    vli_bytesToNative(pub.y, pub_key + uECC_BYTES);

#if (uECC_CURVE == uECC_secp160r1)
    // Don't regularize the bitcount for secp160r1.
    EccPoint_mult(&product, &pub, priv, initial_Z, vli_numBits(priv, uECC_WORDS));
#else
    // Regularize the bitcount for the priv key so that attackers cannot use a side channel
    // attack to learn the number of leading zeros.
    carry = vli_add(priv, priv, curve_n);
    vli_add(tmp, priv, curve_n);
    EccPoint_mult(&product, &pub, p2[!carry], initial_Z, (uECC_BYTES * 8) + 1);
#endif
...
void uECC_compress(const uint8_t pub_key[uECC_BYTES*2], uint8_t compressed[uECC_BYTES+1]) {
    wordcount_t i;
    for (i = 0; i < uECC_BYTES; ++i) {
        compressed[i+1] = pub_key[i];
    }
    compressed[0] = 2 + (pub_key[uECC_BYTES * 2 - 1] & 0x01);
}
...
void uECC_decompress(const uint8_t compressed[uECC_BYTES+1], uint8_t pub_key[uECC_BYTES*2]) {
    EccPoint point;
    ...
    vli_nativeToBytes(pub_key, point.x);
    vli_nativeToBytes(pub_key + uECC_BYTES, point.y);
}

int uECC_valid_pub_key(const uint8_t pub_key[uECC_BYTES*2]) {
    uECC_word_t tmp1[uECC_WORDS];
    uECC_word_t tmp2[uECC_WORDS];
    EccPoint pub;
    ...
    if (EccPoint_isZero(&pub)) {
        return 0;
    }

    // x and y must be smaller than p.
    if (vli_cmp(curve_p, pub.x) != 1 || vli_cmp(curve_p, pub.y) != 1) {
        return 0;
    }

    vli_modSquare_fast(tmp1, pub.y); /* tmp1 = y^2 */
    curve_x_side(tmp2, pub.x); /* tmp2 = x^3 + ax + b */

    /* Make sure that y^2 == x^3 + ax + b */
    return (vli_cmp(tmp1, tmp2) == 0);
}

int uECC_compute_pub_key(const uint8_t priv_key[uECC_BYTES],
                            uint8_t pub_key[uECC_BYTES * 2]) {
    uECC_word_t priv[uECC_WORDS];
    EccPoint pub;

    vli_bytesToNative(priv, priv_key);

    if (!EccPoint_compute_pub_key(&pub, priv)) {
        return 0;
    }
...
static int uECC_sign_with_k(const uint8_t priv_key[uECC_BYTES],
                            const uint8_t message_hash[uECC_BYTES],
    ...
    /* Prevent side channel analysis of vli_modInv() to determine
       bits of k / the priv key by premultiplying by a random number */
    ...
    vli_bytesToNative(tmp, priv_key); /* tmp = d */
    s[uECC_N_WORDS - 1] = 0;
    vli_set(s, p.x);
...
int uECC_sign(const uint8_t priv_key[uECC_BYTES],
              const uint8_t message_hash[uECC_BYTES],
              uint8_t signature[uECC_BYTES*2]) {
    uECC_word_t k[uECC_N_WORDS];
    uECC_word_t tries;
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        if(g_rng_function((uint8_t *)k, sizeof(k))) {
            ...
            k[uECC_WORDS] &= 0x01;
        #endif
            if (uECC_sign_with_k(priv_key, message_hash, k, signature)) {
                return 1;
            }
        }
    }
...
int uECC_sign_deterministic(const uint8_t priv_key[uECC_BYTES],
                            const uint8_t message_hash[uECC_BYTES],
                            uECC_HashContext *hash_context,
                            uint8_t signature[uECC_BYTES*2]) {
    ...
    HMAC_update(hash_context, priv_key, uECC_BYTES);
    ...
    for (tries = 0; tries < MAX_TRIES; ++tries) {
        ...
        if (uECC_sign_with_k(priv_key, message_hash, T, signature)) {
            return 1;
        }
...
int uECC_verify(const uint8_t pub_key[uECC_BYTES*2],
                const uint8_t hash[uECC_BYTES],
                const uint8_t signature[uECC_BYTES*2]) {
